...
## 摘要
尽管在训练目标、架构和数据模态上存在差异，语言模型似乎越来越多地学习到相似的表示。这种独立训练模型之间新兴的兼容性，为跨模型对齐下游目标提供了新的机遇。此外，它开启了新的潜在应用领域，例如在安全性、隐私或竞争约束禁止直接共享数据或模型的场景中。在这项工作中，我们提出了一个隐私保护框架，利用表示收敛来实现独立语言模型之间的跨孤岛推理。该框架在共享的公开数据集上学习仿射变换，并应用同态加密来保护推理过程中的客户端查询。通过仅对线性对齐和分类操作进行加密，该方法在保持强大安全保证的同时，实现了亚秒级的推理延迟。我们通过对表示收敛的实证研究来支持这一框架，在研究中我们学习了独立模型最终隐藏状态之间的线性变换。我们在嵌入分类和分布外检测上评估了这些跨模型映射，观察到模型对之间的性能下降极小。此外，我们首次展示了线性对齐有时能够实现跨独立训练模型的文本生成。
