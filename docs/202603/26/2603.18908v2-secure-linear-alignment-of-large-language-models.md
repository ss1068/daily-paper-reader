...
tldr: 本研究利用大语言模型表示空间的收敛性，提出了一种保护隐私的跨模型对齐框架。该框架通过在公共数据集上学习仿射变换，并结合同态加密技术保护推理过程中的客户端查询，实现了独立模型间的安全协作。实验证明，该方法在保持亚秒级推理延迟的同时，能有效完成嵌入分类、离群检测，甚至实现了跨模型的文本生成，为隐私受限场景下的模型集成提供了高效且安全的新方案。
selection_source: fresh_fetch
motivation: 针对隐私、安全或竞争限制下无法直接共享数据或模型的问题，探索利用模型表示的相似性实现跨机构的安全协作。
method: 提出一种基于线性对齐和同态加密的框架，通过对线性变换和分类操作进行加密，在保护隐私的前提下实现跨模型推理。
result: 实验显示该方法在分类和离群检测任务中性能损失极小，且首次证明了线性对齐可支持独立训练模型间的文本生成。
conclusion: 线性对齐是实现独立大语言模型间安全集成的一种高效且可行的方法，充分利用了模型间天然的表示收敛特性。
---
