...
method: 提出一种灰盒测试方法VeriGrey，利用工具调用序列作为反馈，并通过变异提示词将恶意注入任务与智能体目标挂钩。
result: "在AgentDojo基准测试中，VeriGrey比黑盒基线多发现33%的间接提示词注入漏洞，并在多个真实智能体案例中实现了极高的攻击成功率。"
conclusion: 动态灰盒测试方法对于揭示LLM智能体的安全风险至关重要，是构建智能体安全保障框架的有效途径。
---

## 摘要
智能体 AI（Agentic AI）近期成为了备受关注的话题。大语言模型（LLM）智能体在后端包含一个或多个 LLM；在前端，它通过结合 LLM 输出与调用多个外部工具获得的结果来进行自主决策。与外部环境的自主交互引入了关键的安全风险。在本文中，我们提出了一种灰盒方法来探索 LLM 智能体的多样化行为并揭示其安全风险。我们的方法 VeriGrey 使用调用的工具序列作为反馈函数来驱动测试过程。这有助于发现导致智能体出现非预期行为的罕见但危险的工具调用。作为测试过程中的变异算子，我们对提示词（prompt）进行变异以设计有害的注入提示词。这是通过将智能体的任务与注入任务相链接来精心完成的，从而使注入任务成为完成智能体功能的必要步骤。在著名的 AgentDojo 基准测试上，将我们的方法与黑盒基准进行比较，VeriGrey 在使用 GPT-4.1 后端发现间接提示词注入漏洞方面，效能提升了 33%。我们还对广泛使用的编程智能体 Gemini CLI 和著名的 OpenClaw 个人助手进行了真实案例研究。VeriGrey 发现了诱发多种攻击场景的提示词，而这些场景是黑盒方法无法识别的。在 OpenClaw 中，通过构建一个根据需要采用变异模糊测试的对话智能体，VeriGrey 能够从 10 个恶意技能中发现恶意技能变体（在 Kimi-K2.5 LLM 后端成功率为 100%，在 Opus 4.6 LLM 后端成功率为 90%）。这证明了像 VeriGrey 这样的动态方法在测试智能体方面的价值，并最终有助于构建智能体保障框架。
