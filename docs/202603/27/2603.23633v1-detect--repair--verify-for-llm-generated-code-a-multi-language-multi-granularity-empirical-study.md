...
title_zh: LLM生成代码的“检测-修复-验证”：一项多语言、多粒度的实证研究
authors: Cheng Cheng
date: 2026-03-24
...
## 摘要
大语言模型（LLM）能够生成可运行的软件制品，但其安全性仍难以进行端到端的评估。本研究通过“检测-修复-验证”（DRV）工作流来探讨这一问题，在该工作流中，漏洞被检测、修复，随后通过安全和功能测试进行重新检查。它解决了当前证据中的四个空白：缺乏针对LLM生成制品的基于测试的基准测试、关于流水线级有效性的证据有限、检测报告作为修复指导的可靠性不明，以及验证下修复的可信度不确定。为了支持这项研究，本文构建了 EduCollab，这是一个包含 PHP、JavaScript 和 Python 编写的可运行 LLM 生成 Web 应用程序的多语言、多粒度基准测试。每个制品都配有可执行的功能和漏洞利用测试套件，且该基准测试涵盖了项目级、需求级和文件级设置。在该基准测试上，本研究在相当的预算约束下，比较了未修复的基准线、单次检测-修复以及有界迭代 DRV。结果通过“安全且正确”的产出率进行衡量，并分析了中间制品和迭代轨迹，以评估报告的可操作性和修复失败模式。结果表明，有界迭代 DRV 相比单次修复可以提高“安全且正确”的产出率，但这种提升在项目级别并不均衡，而在更窄的修复范围内则变得更加明显。检测报告通常对下游修复有用，但其可靠性并不一致。修复的可信度也强烈依赖于修复范围。这些发现强调了对基于 LLM 的漏洞管理工作流进行基于测试的端到端评估的必要性。
