...
title_zh: 可观测通道，而非仅是存储：评估大语言模型（LLM）智能体流水线中的隐私泄露
authors: "Tao Huang, Chen Hou, Guosen Wu, Jiayang Meng"
date: 2026-03-24
pdf: "https://arxiv.org/pdf/2603.22751v2"
tags: ["query:llm"]
score: 7.0
evidence: 评估LLM智能体流水线中的隐私泄露
tldr: 本研究针对LLM智能体隐私泄露评估中组件孤立分析的局限性，提出了统一的通道导向测量接口CIPL。该框架通过敏感源、选择、组装、执行、观察和提取六个阶段，系统评估了内存、检索和工具介导的流水线。研究发现，隐私泄露不仅取决于存储，更受可观测通道条件的制约。这一工作为理解异构智能体流水线中的隐私风险提供了新视角，强调了通道导向评估在隐私保护中的必要性。
selection_source: fresh_fetch
motivation: 现有的LLM智能体隐私研究多关注孤立组件，难以统一衡量和比较不同流水线中内部敏感信息向外部泄露的转化过程。
method: 提出CIPL框架，将智能体操作抽象为统一的通道模型，通过模拟攻击者从观察到的输出中反推内部敏感源来评估泄露程度。
result: 内存表现为高风险饱和状态，检索介导的泄露虽频繁但往往不完整，而工具介导的泄露则高度依赖于暴露的观察面。
conclusion: LLM智能体的隐私风险应被视为可观测通道的属性而非单纯的存储问题，通道导向的评估是全面保障隐私的关键。
---

## 摘要
LLM 智能体中的隐私泄露通常通过单个存储或执行组件进行研究，例如记忆模块、检索流水线或工具介导的伪影。然而，这些设置通常是孤立分析的，因此难以比较在异构智能体流水线中，私有的内部依赖如何转变为外部可恢复的泄露。在本文中，我们提出了 CIPL（隐私泄露通道反转），作为一种统一的、面向通道的测量接口，用于评估 LLM 智能体流水线中的隐私泄露。CIPL 并非旨在声称一种普遍最强的攻击方案，而是提供了一种通用的方式，通过敏感源、选择、组装、执行、观测和提取阶段来表示目标，并在通用协议下测量内部暴露如何转化为攻击者可恢复的泄露。通过在该通用接口下使用基于记忆、检索介导和工具介导的实例化，我们识别出了一种独特的跨目标风险图景。记忆表现为一种近乎饱和的高风险特例，而记忆之外的泄露则呈现出不同的模式：检索介导的目标显示出频繁但通常不完整的泄露，而工具介导的目标则深受暴露的观测面和提供者行为的影响。我们进一步表明，泄露受通道条件的支配，而非由某种普遍主导的方案决定：经过清理的弱控制能显著抑制泄露，而语义标注则揭示了超出精确匹配提取之外的、对攻击者有用的泄露。总之，这些发现表明，通过“可观测通道”而非仅仅是存储组件，可以更好地理解 LLM 智能体流水线中的隐私风险。更广泛地说，我们的结果表明，面向通道的隐私评估是组件局部或仅限精确匹配分析的必要补充。
