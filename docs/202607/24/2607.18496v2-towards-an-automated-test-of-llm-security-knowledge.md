...
tldr: 本研究针对大语言模型（LLM）在安全任务中知识评估成本高、依赖人工的问题，提出了一种半自动化的测试方法。该方法利用消费者保护机构（CPA）的权威数据，通过分析模型在处理特定安全叙述时的响应不稳定性，来识别其知识盲区。在对Gemini和GPT系列模型的实验中，该方法有效评估了模型对身份盗窃和冒充诈骗的识别能力，为自动化衡量LLM安全专业知识提供了新思路。
selection_source: fresh_fetch
motivation: 现有的LLM安全知识评估方法高度依赖人工设计和专家经验，缺乏高效的自动化评估手段。
method: 提出一种利用消费者保护机构的权威信息，通过检测模型响应的不稳定性来识别其安全知识差距的半自动化方法。
result: 在身份盗窃和冒充诈骗主题下对Gemini和GPT系列模型进行测试，成功区分了模型在特定安全领域的知识掌握程度。
conclusion: 该方法证明了利用外部权威数据自动评估LLM安全知识的可行性，降低了构建安全基准测试的门槛。
---
