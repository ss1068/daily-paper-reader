...
tags: ["query:llm"]
score: 8.0
evidence: 针对大模型智能体提示词注入的深度防御运行时安全层
...
motivation: 工具增强型 LLM 智能体面临间接提示注入、凭据泄露和不安全工具调用等严峻安全威胁，现有的用户输入过滤手段已不足以应对。
method: 提出 PRISM 架构，通过 10 个生命周期钩子集成启发式与 LLM 混合扫描、会话级风险累积以及针对工具、网络和机密的策略强制执行。
result: 初步基准测试表明，该系统在保持低运行时开销和高操作恢复能力的同时，能有效拦截多种针对智能体运行时的攻击。
conclusion: OpenClaw PRISM 为实际部署的智能体网关提供了一个可扩展、防篡改且支持热加载策略的深度防御安全层。
---
