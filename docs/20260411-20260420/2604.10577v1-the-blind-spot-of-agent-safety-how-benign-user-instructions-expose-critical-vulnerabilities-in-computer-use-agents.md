...
## 摘要
计算机操作智能体（CUA）目前已能自主在真实数字环境中完成复杂任务，但在被误导时，它们也可能被用于以编程方式自动执行有害行为。现有的安全评估主要针对滥用和提示词注入等显式威胁，却忽视了一种微妙且关键的情境：即用户指令完全良性，而危害源于任务上下文或执行结果。我们提出了 OS-BLIND，这是一个在非预期攻击条件下评估 CUA 的基准测试，包含 300 个由人工设计的任务，涵盖 12 个类别、8 个应用程序和 2 个威胁集群：环境嵌入式威胁和智能体发起的危害。我们对前沿模型和智能体框架的评估显示，大多数 CUA 的攻击成功率（ASR）超过 90%，即使是经过安全对齐的 Claude 4.5 Sonnet，其 ASR 也达到了 73.0%。更值得注意的是，当 Claude 4.5 Sonnet 部署在多智能体系统中时，这一漏洞变得更加严重，ASR 从 73.0% 上升至 92.7%。我们的分析进一步表明，当用户指令为良性时，现有的安全防御措施保护作用有限。安全对齐主要在最初的几个步骤中激活，在随后的执行过程中很少再次介入。在多智能体系统中，分解后的子任务掩盖了模型的有害意图，导致经过安全对齐的模型失效。我们将发布 OS-BLIND，以鼓励更广泛的研究社区进一步调查并解决这些安全挑战。
