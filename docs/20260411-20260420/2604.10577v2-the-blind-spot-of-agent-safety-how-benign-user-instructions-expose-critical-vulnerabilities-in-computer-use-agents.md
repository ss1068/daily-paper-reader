...
tags: ["query:llm"]
score: 6.0
evidence: 计算机使用智能体的安全评估与漏洞
tldr: 本研究揭示了计算机操作智能体（CUA）在用户指令良性但执行环境或结果有害时的安全盲点。作者提出了OS-BLIND基准测试，包含300个涵盖环境嵌入威胁和智能体引发危害的任务。实验发现，即使是经过安全对齐的前沿模型（如Claude 4.5 Sonnet），在单智能体和多智能体系统中的攻击成功率（ASR）依然极高。研究指出，现有防御机制难以识别被分解子任务掩盖的有害意图，为CUA的安全研究提供了重要参考。
selection_source: fresh_fetch
motivation: 现有的智能体安全评估主要关注显式恶意指令，忽视了良性指令在特定执行环境下可能引发的严重安全风险。
method: 提出了OS-BLIND基准测试，通过300个人工构建的涵盖12个类别和8种应用的真实场景任务，评估智能体在非预期攻击条件下的安全性。
result: "实验显示前沿模型在OS-BLIND上的攻击成功率普遍超过90%，且多智能体协作会因任务分解进一步掩盖有害意图，导致安全防御能力显著下降。"
conclusion: 现有的安全对齐和防御手段在处理良性指令诱导的危害时存在严重局限，亟需开发能持续监控执行过程并识别隐蔽意图的新型安全机制。
---

## 摘要
计算机操作智能体（CUA）现在能够在真实的数字环境中自主完成复杂任务，但一旦被误导，它们也可能被用于以编程方式自动执行有害操作。现有的安全评估主要针对滥用和提示词注入等显式威胁，但忽视了一个微妙且关键的场景：即用户指令完全良性，而危害源于任务上下文或执行结果。我们推出了 OS-BLIND，这是一个在非预期攻击条件下评估 CUA 的基准测试，包含跨 12 个类别、8 个应用程序和 2 个威胁集群（环境嵌入式威胁和智能体发起的危害）的 300 个由人工设计的任务。我们对前沿模型和智能体框架的评估显示，大多数 CUA 的攻击成功率（ASR）超过 90%，即使是经过安全对齐的 Claude 4.5 Sonnet，其 ASR 也达到了 73.0%。更有趣的是，当 Claude 4.5 Sonnet 部署在多智能体系统中时，这种脆弱性变得更加严重，ASR 从 73.0% 上升到 92.7%。我们的分析进一步表明，当用户指令为良性时，现有的安全防御措施提供的保护有限。安全对齐主要在最初的几个步骤中激活，在随后的执行过程中很少再次介入任务。在多智能体系统中，分解后的子任务掩盖了模型的有害意图，导致经过安全对齐的模型失效。我们将发布 OS-BLIND，以鼓励更广泛的研究社区进一步调查并解决这些安全挑战。
