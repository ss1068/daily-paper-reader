...
result: "在带有副作用的任务中，YoloFS助力智能体在约72%的场景下实现自纠错，并在常规任务中以更低的用户交互频率保持了同等成功率。"
conclusion: 通过将信息反馈和控制权下沉至文件系统层，可以有效解决AI智能体文件操作的安全隐患并增强其独立工作能力。
---

## 摘要
AI 编程智能体直接在用户的文件系统上运行，常会造成数据损坏、文件删除和密钥泄露。目前的方法被迫在安全性与自主性之间进行权衡：不受限制的访问存在风险，而频繁的权限提示则会加重用户负担并阻碍智能体运行。为了深入理解这一问题，我们对智能体文件系统误用情况进行了首次系统性研究，分析了 13 个框架中的 290 份公开报告。分析表明，目前的智能体对其文件系统操作的影响了解有限，且缺乏足够的控制能力。因此，我们主张将这些信息和控制权转移到文件系统本身。基于这一原则，我们设计了 YoloFS，这是一种采用三种技术的智能体原生文件系统。暂存（Staging）在提交前隔离所有变更，为用户提供纠正性控制；快照（Snapshots）将这种控制权扩展到智能体，使其能够检测并纠正自身错误；渐进式权限（Progressive permission）通过以最少的交互限制访问，为用户提供预防性控制。为了评估 YoloFS，我们引入了一种捕获用户-智能体-文件系统交互的新方法。在 11 个具有隐藏副作用的任务中，YoloFS 使智能体在 8 个任务中实现了自我纠正，同时保持所有影响处于暂存状态且可供审查。在 112 个常规任务中，YoloFS 在保持与基准相当的成功率的同时，减少了用户交互。
