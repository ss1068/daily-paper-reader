...
## 摘要
跨组织边界部署的检索增强生成 (RAG) 系统在安全性、准确性和效率之间面临着根本性的权衡。目前的加密方法在解密过程中会暴露明文，而联邦架构则阻碍了资源整合并产生了巨大的开销。我们提出了 Trans-RAG，它实现了一种新颖的向量空间语言范式，使每个组织的知识都存在于数学隔离的语义空间中。其核心是 vector2Trans，这是一种多阶段变换技术，使查询能够通过以查询为中心的变换动态地“说出”每个组织的向量空间“语言”，在消除解密开销的同时保持了原生的检索效率。安全性评估表明，向量空间接近正交，角度间隔为 89.90°，隔离率达到 99.81%。在 8 个检索器、3 个数据集和 3 个大语言模型 (LLM) 上的实验表明，准确率下降极小（nDCG@10 下降 3.5%），且相比同态加密在效率上有显著提升。
