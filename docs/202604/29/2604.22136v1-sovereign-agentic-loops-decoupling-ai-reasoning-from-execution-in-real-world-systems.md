...
tags: ["query:jb-def"]
score: 7.0
evidence: 针对安全策略验证模型意图的控制平面架构
tldr: 针对大语言模型智能体在执行系统操作时存在的安全风险，本文提出了主权代理循环（SAL）架构。该架构通过将AI推理与执行解耦，要求模型输出结构化意图及理由，并由控制平面依据真实系统状态和策略进行验证。SAL引入了混淆膜和证据链技术，确保了身份隔离与可审计性。实验表明，该系统能有效拦截不安全意图，且延迟极低，为构建安全可靠的自主智能体系统提供了新方案。
selection_source: fresh_fetch
motivation: 现有的智能体架构直接执行模型输出的随机指令，缺乏对系统状态和安全策略的有效验证，存在严重的安全隐患。
method: 提出SAL架构，通过控制平面验证模型意图，并结合混淆膜保护敏感信息及证据链实现操作的可追溯性。
result: 在云基础设施原型测试中，SAL成功拦截了所有不安全意图，且中位延迟仅增加12.4毫秒。
conclusion: SAL架构通过解耦推理与执行，在不显著影响性能的前提下，为真实世界系统中的AI智能体提供了策略约束和安全保障。
---
