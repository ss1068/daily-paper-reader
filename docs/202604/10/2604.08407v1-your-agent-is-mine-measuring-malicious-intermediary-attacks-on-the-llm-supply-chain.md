...
title_zh: 你的智能体归我了：衡量大语言模型供应链中的恶意中间人攻击
authors: "Hanzhi Liu, Chaofan Shou, Hongbo Wen, Yanju Chen, Ryan Jingyang Fang, Yu Feng"
date: 2026-04-09
pdf: "https://arxiv.org/pdf/2604.08407v1"
tags: ["query:llm"]
score: 8.0
evidence: LLM供应链中的有效载荷注入攻击
tldr: 本研究首次系统性地探讨了LLM供应链中第三方API路由器的安全风险。研究定义了恶意路由器的威胁模型，涵盖载荷注入和秘密窃取等攻击类别。通过对428个付费和免费路由器的实测，发现大量路由器存在注入恶意代码、窃取凭据及加密货币等行为。研究还通过诱饵实验揭示了该生态系统的脆弱性，并提出了三种客户端防御机制，旨在提升LLM代理的安全性。
selection_source: fresh_fetch
motivation: 第三方LLM API路由器作为拥有明文访问权限的代理，在缺乏加密完整性保护的情况下构成了严重的供应链安全威胁。
method: 形式化定义了恶意路由器的攻击模型，并对数百个真实路由器进行实测，同时通过诱饵实验和自研代理Mine评估攻击效果与防御措施。
result: 实测发现多个路由器存在注入恶意代码、窃取AWS凭据或ETH的行为，且诱饵实验显示弱配置系统极易遭受大规模Token盗刷和凭据泄露。
conclusion: LLM代理生态系统面临严峻的中间人攻击风险，必须通过策略网关、异常筛选和透明度日志等防御手段来增强安全性。
---

## 摘要
大语言模型 (LLM) 智能体越来越多地依赖第三方 API 路由在多个上游提供商之间分发工具调用请求。这些路由作为应用层代理运行，拥有对传输中每个 JSON 负载的完整明文访问权限，然而目前没有任何提供商在客户端与上游模型之间强制执行加密完整性校验。我们对这一攻击面进行了首次系统性研究。我们为恶意 LLM API 路由建立了一个威胁模型，并定义了两类核心攻击：负载注入 (AC-1) 和密钥窃取 (AC-2)，以及两种自适应规避变体：针对依赖项的注入 (AC-1.a) 和条件交付 (AC-1.b)。通过对购自淘宝、闲鱼和 Shopify 托管店铺的 28 个付费路由，以及从公共社区收集的 400 个免费路由进行调查，我们发现有 1 个付费路由和 8 个免费路由正在主动注入恶意代码，2 个部署了自适应规避触发器，17 个触碰了研究人员拥有的 AWS 金丝雀凭据，还有 1 个从研究人员拥有的私钥中窃取了以太坊 (ETH)。两项投毒研究进一步表明，表面上良性的路由也可能被卷入同样的攻击面：一个泄露的 OpenAI 密钥产生了 1 亿个 GPT-5.4 token 和超过 7 个 Codex 会话，而配置薄弱的诱饵则产生了 20 亿个计费 token、分布在 440 个 Codex 会话中的 99 个凭据，以及 401 个已经以自主 YOLO 模式运行的会话。我们构建了 Mine，这是一个针对四个公共智能体框架实现所有四类攻击的研究代理，并利用它评估了三种可部署的客户端防御措施：故障关闭策略网关、响应端异常筛选以及仅追加透明度日志。
