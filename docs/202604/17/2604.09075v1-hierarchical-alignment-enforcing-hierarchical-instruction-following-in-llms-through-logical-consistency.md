...
title_zh: 层级对齐：通过逻辑一致性在大型语言模型中实现层级指令遵循
authors: "Shu Yang, Zihao Zhou, Di Wang, Wenda Li"
date: 2026-04-10
pdf: "https://arxiv.org/pdf/2604.09075v1"
tags: ["query:llm"]
score: 7.0
evidence: 在层级指令遵循中避免安全违规
tldr: 针对大语言模型在处理来自系统、用户、工具等不同优先级指令时出现的冲突问题，本文提出神经符号层次对齐（NSHA）方法。该方法在推理阶段将指令解析建模为约束满足问题，利用求解器引导模型推导出最大一致的指令集，并在训练阶段通过蒸馏求解器决策来优化模型参数。实验证明，NSHA在处理指令冲突、任务执行和安全性方面表现优异，同时保持了良好的通用能力。
selection_source: fresh_fetch
motivation: 现有研究多关注对抗性攻击，忽视了实际应用中常见的良性指令冲突对模型任务效用和行为一致性的影响。
method: 提出NSHA框架，在推理时采用求解器引导的逻辑推理解决指令冲突，在训练时通过自动构建的监督数据将逻辑决策能力蒸馏至模型。
result: 在规则遵循、工具使用和安全性等多个维度的评估中，NSHA显著提升了模型在指令冲突场景下的性能，并保持了极具竞争力的基础效用。
conclusion: 通过显式建模指令优先级并结合逻辑一致性约束，可以有效增强大语言模型在复杂多源指令环境下的层次化遵循能力。
---

## 摘要
大型语言模型越来越多地在来自不同权威级别的异构来源的多重指令下运行，包括系统策略、用户请求、工具输出和检索到的上下文。虽然先前关于指令层级的工作强调了尊重指令优先级的重要性，但其主要关注对抗性攻击，而忽视了现实应用中出现的良性但常见的指令冲突。在这种情况下，当指令发生部分或隐式冲突时，模型不仅必须避免安全违规，还必须保持任务效用和行为一致性。我们提出了神经符号层级对齐（NSHA），通过显式建模和强制执行指令优先级来实现层级指令遵循。在推理阶段，我们引入了求解器引导的推理，将指令解析建模为一个约束满足问题，使模型能够在层级约束下推导出最大一致的适用指令集。在训练阶段，NSHA 利用自动构建的监督信号，将基于求解器的决策蒸馏到模型参数中。我们在规则遵循、任务执行、工具使用和安全性方面评估了我们的方法，涵盖了单轮和多轮交互，并表明 NSHA 在此类冲突下显著提高了性能，同时在参考设置中保持了具有竞争力的效用。
