...
tags: ["query:llm"]
score: 10.0
evidence: 用于越狱攻击的结构化语义掩蔽
tldr: 针对现代大语言模型能识别深层语义意图并拒绝恶意请求的问题，本文提出结构化语义掩蔽（S2C）越狱攻击框架。该框架通过上下文重构、内容碎片化和线索引导伪装，策略性地分散语义线索，迫使模型在推理时延迟整合恶意意图，从而绕过安全防御。实验显示，S2C在多个主流模型上显著提升了攻击成功率，超越了现有最先进方法，揭示了模型在处理复杂语义重构时的安全漏洞。
selection_source: fresh_fetch
motivation: 现代LLM的安全机制已能识别深层语义意图，导致传统的表面混淆式越狱攻击手段逐渐失效。
method: 提出S2C框架，利用上下文重构、内容碎片化和线索引导伪装三种机制，在模型推理过程中延迟并重构语义整合。
result: "在HarmBench和JBB-Behaviors测试中，S2C的攻击成功率分别比SOTA提高12.4%和9.7%，在GPT-4o-mini上提升尤为显著。"
conclusion: S2C通过操纵语义整合过程有效绕过了LLM的深层安全防御，证明了结构化语义混淆在越狱攻击中的强大潜力。
---
