...
## 摘要
近期的智能系统通过 API 集成了强大的大语言模型（LLM），但其可信度可能会受到后门攻击和提示词注入攻击等针对性攻击的严重破坏，这些攻击会秘密地迫使 LLM 生成特定的恶意序列。现有的针对此类威胁的防御方法通常依赖于高访问权限，产生高昂的成本，并阻碍正常的推理，使其在现实场景中不切实际。为了解决这些局限性，我们引入了 DualSentinel，这是一个轻量级且统一的防御框架，能够在 LLM 生成过程中准确且及时地检测到针对性攻击的激活。我们首先识别了受攻击 LLM 的一个特征，称为“熵平稳”（Entropy Lull）：当针对性攻击成功劫持生成过程时，LLM 会表现出一段明显的、异常低且稳定的 Token 概率熵，这表明它正在遵循一条固定路径，而不是进行创造性的选择。DualSentinel 通过开发一种创新的双重检查方法来利用这一模式。它首先采用一种感知幅度和趋势的监测方法，在运行时主动且灵敏地标记熵平稳模式。一旦标记，它会触发一种基于任务翻转（task-flipping）的轻量级且强大的二次验证。只有当熵平稳模式在原始任务和翻转任务中都持续存在时，攻击才会被确认，从而证明 LLM 的输出受到了强制控制。广泛的评估表明，DualSentinel 既高效（具有卓越的检测准确率和近乎零的误报率），又非常经济（额外成本可以忽略不计），为保障已部署 LLM 的安全提供了一条真正实用的路径。源代码可通过 https://doi.org/10.5281/zenodo.18479273 访问。
