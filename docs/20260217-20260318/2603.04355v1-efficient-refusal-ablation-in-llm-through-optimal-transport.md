...
tldr: 本研究提出了一种基于最优传输（Optimal Transport）理论的LLM拒绝机制消融框架，旨在克服现有方法仅移除单一拒绝方向的局限性。通过结合PCA与高斯最优传输，该方法能将有害请求的激活分布整体转换为无害分布。在Llama和Qwen系列模型上的实验证明，该方法在显著提升越狱成功率的同时，能更好地保持模型的语言生成能力，并揭示了拒绝机制在模型中间层的局部化特性。
selection_source: fresh_fetch
motivation: 现有的激活层越狱方法通常将拒绝行为简化为一维向量移除，忽略了模型内部激活值复杂的分布结构。
method: 利用主成分分析与闭式高斯最优传输理论，实现高维空间中有害与无害激活分布的高效对齐与转换。
result: "在多个主流大模型上，该方法比现有基准提升了最高11%的攻击成功率，且具有更优的模型困惑度表现。"
conclusion: "拒绝机制在LLM中并非完全分布式，而是集中在网络约40-60%深度的特定层，且当前的对齐防御易受分布级攻击的影响。"
---
