...
tldr: 针对LLM智能体易受间接提示注入（IPI）攻击且现有防御易导致过度拒绝的问题，本文提出ICON框架。该框架通过探测潜空间中的“过度关注”特征来识别攻击，并利用缓解整流器进行精确的注意力引导，在消除恶意指令影响的同时保留任务连续性。实验表明ICON在保持极低攻击成功率的同时，显著提升了任务效能，并具备良好的跨模态和分布外泛化性。
selection_source: fresh_fetch
motivation: 现有的间接提示注入防御机制往往因过度拒绝而中断合法的智能体工作流，亟需在安全性和任务可用性之间取得平衡。
method: 提出ICON框架，利用潜空间追踪探测器识别攻击特征，并通过缓解整流器对注意力权重进行手术式修正，以恢复LLM的正常功能轨迹。
result: "在多个模型上实现了0.4%的极低攻击成功率，同时相比商业探测器提升了超过50%的任务效能，并展现出优异的泛化能力。"
conclusion: ICON通过推理侧修正而非简单拒绝，为LLM智能体提供了一种兼顾高安全性和高实用性的防御新范式。
---

## 摘要
大语言模型 (LLM) 智能体容易受到间接提示注入 (IPI) 攻击，即检索内容中的恶意指令劫持了智能体的执行。现有的防御措施通常依赖于严格的过滤或拒绝机制，这面临一个关键限制：过度拒绝，导致有效的智能体工作流过早终止。我们提出了 ICON，一个从探测到缓解的框架，在保持任务连续性的同时中和攻击。我们的核心见解是，IPI 攻击在潜空间中留下了明显的过度聚焦特征。我们引入了一个潜空间轨迹探测器 (Latent Space Trace Prober)，根据高强度得分来检测攻击。随后，缓解整流器 (Mitigating Rectifier) 执行手术级注意力引导，选择性地操纵对抗性查询-键 (query-key) 依赖关系，同时放大任务相关元素，以恢复 LLM 的功能轨迹。在多个主干网络上的广泛评估表明，ICON 实现了 0.4% 的竞争性攻击成功率 (ASR)，与商业级检测器相当，同时带来了超过 50% 的任务效用提升。此外，ICON 展示了鲁棒的分布外 (OOD) 泛化能力，并能有效扩展到多模态智能体，在安全性和效率之间建立了卓越的平衡。
