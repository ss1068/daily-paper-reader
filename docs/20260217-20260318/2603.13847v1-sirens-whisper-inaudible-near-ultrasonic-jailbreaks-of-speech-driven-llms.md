...
tags: ["query:llm"]
score: 10.0
evidence: 语音驱动大模型的近超声波隐形越狱
tldr: "本研究提出了Sirens' Whisper (SWhisper)，这是首个针对语音驱动大语言模型（LLM）的隐蔽近超声波越狱攻击框架。该框架利用商用硬件的麦克风非线性效应，将恶意指令编码为人类听不见的近超声波信号，在现实黑盒环境下实现高保真指令注入。实验证明，该方法能有效绕过商业和开源模型的安全限制，且在人类听觉测试中具有极高的隐蔽性，为语音交互安全提供了新视角。"
selection_source: fresh_fetch
motivation: 语音驱动的LLM通过开放声学通道交互，面临着利用近超声波进行隐蔽指令注入的安全威胁。
method: 利用麦克风非线性解调原理，结合信道建模与预补偿技术，将语音越狱指令调制为不可听见的近超声波。
result: 在商业模型上取得了高达0.94的攻击成功率，且用户研究证实攻击信号对人类完全不可察觉。
conclusion: 该研究证明了通过物理声学通道对语音LLM实施高保真、隐蔽越狱攻击的现实可行性与严重威胁。
---
