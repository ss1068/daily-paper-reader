...
tldr: 本研究深入探讨了大语言模型中由“续写”指令触发的越狱现象，即通过改变指令后缀位置显著提升攻击成功率。研究者利用机理解释性分析，通过因果干预和激活缩放等手段，揭示了该现象源于模型内在续写驱动力与安全对齐防御之间的竞争。通过识别并分析不同架构下的安全关键注意力头，本文为理解越狱机制提供了新视角，并为增强模型安全性提供了理论支持。
selection_source: fresh_fetch
motivation: 旨在揭示为何简单的续写指令后缀重定位能显著提高大语言模型的越狱成功率及其背后的深层机理。
method: 采用基于注意力头层面的机理解释性分析，结合因果干预、激活缩放和行为分析来探究模型内部机制。
result: 发现越狱行为本质上是模型固有的续写驱动力与安全对齐防御机制之间竞争的结果，并识别出关键的安全注意力头。
conclusion: 研究从机理层面解释了续写触发的越狱行为，为理解模型脆弱性及改进安全防御策略提供了重要的理论依据和实践启示。
---
