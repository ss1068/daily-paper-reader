...
tags: ["query:llm"]
score: 10.0
evidence: 针对对抗性越狱攻击的推理时防御
tldr: 针对大语言模型易受越狱攻击且现有防御手段存在局限性的问题，本文提出 MANATEE，一种推理阶段的轻量级防御方法。它通过学习良性表示流形的密度分布，利用扩散模型将异常隐藏状态投影回安全区域。该方法无需有害数据训练或架构修改，在 Mistral、Llama 和 Gemma 等模型上显著降低了攻击成功率，同时保留了模型的通用能力。
selection_source: fresh_fetch
motivation: 现有的 LLM 防御方法依赖于易失效的分类器或成本高昂且可能损害模型性能的微调手段。
method: MANATEE 利用扩散模型学习良性隐藏状态的评分函数，并在推理时将异常表示投影至安全流形。
result: "实验证明该方法在多个主流模型上能显著降低越狱攻击成功率，部分数据集下防御率达 100%。"
conclusion: MANATEE 是一种高效且无需修改架构的推理端防御方案，为提升 LLM 安全性提供了新思路。
---

## 摘要
保护大语言模型（LLMs）免受对抗性越狱攻击仍是一个公开的挑战。现有的防御措施依赖于二元分类器，当对抗性输入超出学习到的决策边界时，这些分类器就会失效；此外，重复的微调计算成本高昂，且可能降低模型能力。我们提出了 MANATEE，这是一种在推理阶段进行的防御方法，它在良性表示流形（benign representation manifold）上使用密度估计。MANATEE 学习良性隐藏状态的评分函数（score function），并利用扩散过程将异常表示投影到安全区域——该方法不需要有害的训练数据，也不需要修改模型架构。在 Mistral-7B-Instruct、Llama-3.1-8B-Instruct 和 Gemma-2-9B-it 上的实验表明，MANATEE 在某些数据集上能将攻击成功率（Attack Success Rate）降低高达 100%，同时在良性输入上保持了模型的效用。
