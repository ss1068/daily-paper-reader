...
tags: ["query:llm"]
score: 8.0
evidence: 工具增强型大模型智能体的运行时安全层
tldr: OpenClaw PRISM 是针对工具增强型 LLM 智能体的零分叉运行时安全层，旨在解决间接提示注入、不安全工具执行和凭据泄露等风险。它通过在智能体生命周期的 10 个关键钩子处部署防御，结合启发式与 LLM 扫描、风险累积机制及严格的策略控制，提供深度防御。该系统强调实际部署中的安全性、审计完整性和低运行时开销，为智能体网关提供了稳健的安全保障。
selection_source: fresh_fetch
motivation: 现有的 LLM 智能体面临间接提示注入、凭据泄露和不安全工具调用等严峻的安全威胁，亟需一种非侵入式的运行时防御机制。
method: 提出 PRISM 架构，通过 10 个生命周期钩子集成启发式与 LLM 混合扫描、会话级风险累积以及针对工具、网络和机密的策略强制执行。
result: 初步基准测试显示，该系统在保持低运行时开销的同时，能有效检测并拦截多种攻击向量，并具备良好的操作恢复能力。
conclusion: OpenClaw PRISM 为工具增强型智能体提供了一个可部署的、深度防御的安全层，平衡了安全有效性与系统运行效率。
---

## 摘要
工具增强型 LLM 智能体引入了超出用户输入过滤范围的安全风险，包括通过获取内容进行的间接提示词注入、不安全的工具执行、凭据泄露以及对本地控制文件的篡改。我们提出了 OpenClaw PRISM，这是一个为基于 OpenClaw 的智能体网关设计的零分叉（zero-fork）运行时安全层。PRISM 将进程内插件与可选的边车（sidecar）服务相结合，并将执行策略分布在涵盖消息入口、提示词构建、工具执行、工具结果持久化、出站消息传递、子智能体生成以及网关启动的十个生命周期钩子中。PRISM 并非引入一种全新的检测模型，而是集成了一个混合启发式加 LLM 的扫描流水线、具有基于 TTL 衰减的对话和会话范围风险累积机制、针对工具、路径、私有网络、域名层级和出站机密模式的策略强制控制，以及一个具有完整性验证和热重载策略管理的防篡改审计与操作平面。我们概述了一种评估方法和基准测试流水线，用于衡量智能体运行时环境下的安全有效性、误报率、各层贡献、运行时开销和操作恢复能力，并报告了在精选的同切片实验和操作微基准测试上的当前初步基准测试结果。该系统旨在为真实的智能体网关提供可部署的运行时防御，而非仅用于基准测试的检测。
