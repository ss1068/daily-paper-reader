...
title_zh: TriShield：通过正交梯度投影与优化器状态纠缠实现的联邦语言模型微调中零效用损失的隐私后门防御
authors: Cheng Wei
date: 2026-07-30
pdf: "https://arxiv.org/pdf/2607.27940v2"
tags: ["query:llm"]
score: 6.0
evidence: 联邦大模型微调中针对隐私后门的防御
tldr: "针对联邦学习微调大模型中恶意服务器通过隐私后门重构客户端私有数据的问题，本文提出TriShield防御框架。该框架包含参数伪影检测、状态虚拟迭代和零效用正交投影三层机制，在不损失模型性能且不增加通信轮数的前提下，将数据重构率降至0%，有效保障了联邦微调的隐私安全。"
selection_source: fresh_fetch
motivation: 现有的联邦学习隐私防御手段在应对新型隐私后门攻击时，往往会导致模型效用大幅下降，难以平衡安全与性能。
method: TriShield通过检测异常神经元、利用优化器状态纠缠梯度以及将梯度投影至主任务语义子空间，从物理和数学层面阻断隐私泄露。
result: "实验表明该方法在保持甚至提升训练精度的同时，将攻击重构率降至0%，且计算开销增加不到5%。"
conclusion: TriShield证明了在联邦微调场景下，可以通过正交投影和状态纠缠实现零效用损失的强隐私保护。
---

## 摘要
大语言模型（LLM）的联邦微调实现了在不暴露原始数据的情况下进行协作训练。然而，最近的一项名为 NeuroImprint 的攻击表明，恶意参数服务器可以将 PEFT 适配器破坏为隐私后门：通过为每个训练样本分配一个专门的记忆神经元，并确保每个神经元最多更新一次，服务器可以以高语义保真度解析地重构 59%--79% 的客户端训练数据。现有的防御措施——包括本地差分隐私（LDP）和梯度裁剪——要么无法对抗这种攻击，要么会造成不可接受的效用退化。我们提出了 TriShield，这是一种三层确定性防御机制，能够在零模型效用损失且无需额外通信轮次的情况下，完全防止 NeuroImprint 式的重构。TriShield 由以下部分组成：(1) 参数伪影检测器（Parameter Artifact Detector），在本地训练开始前识别分布式模型参数中的记忆神经元特征；(2) 有状态虚拟迭代（Stateful Virtual Iteration）机制，强制 Adam/AdamW 的动量状态在虚拟步骤中不可逆地纠缠梯度，从而使 NeuroImprint 的闭式求逆失效；(3) 零效用正交投影（Zero-Utility Orthogonal Projection）算子，将所有本地梯度更新投影到通过 SVD 计算的主任务语义子空间上，从物理上消除任何携带隐私记忆的梯度分量。我们在理论上证明，在经过第 2 层和第 3 层处理后，上传的梯度与任何单个训练样本之间的互信息为零。在 GPT-2 (117M) 和 Llama-Guard-3-1B 上的实验验证了 TriShield 在所有测试的攻击变体中将 NeuroImprint 的重构率降低至 0%，同时保持或提高了训练准确率，且额外的 GPU 计算开销低于 5%。
