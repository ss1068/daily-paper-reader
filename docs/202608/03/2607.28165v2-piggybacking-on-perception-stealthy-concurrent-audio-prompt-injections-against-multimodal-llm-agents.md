...
tags: ["query:llm"]
score: 7.0
evidence: 针对多模态大模型智能体的音频提示注入攻击
tldr: 本研究揭示了多模态大模型智能体在音频交互中的安全隐患，提出了一种隐蔽的并发音频提示注入攻击。该攻击利用指令增强和场景隐藏技术，将恶意指令嵌入用户正常语音中以劫持智能体。通过构建AudioAgentSecurity基准测试，研究证实了攻击的高成功率，并提出了基于源分离的CADV防御机制，有效提升了智能体的安全性。
selection_source: fresh_fetch
motivation: 多模态智能体依赖连续音频交互，这使得环境噪声中的恶意指令注入成为一种隐蔽且危险的攻击面。
method: 提出了一种并发音频注入技术，通过将恶意指令“搭载”在用户语音中实现劫持，并建立了首个综合性攻击基准。
result: "攻击在Gemini 3 Pro上达到了69.10%的成功率，而提出的CADV防御机制在检测此类攻击时达到了96%的准确率。"
conclusion: 研究证明了现有音频交互智能体极易受到隐蔽注入攻击，而采用音频解耦与一致性验证是有效的防御手段。
---

## 摘要
由大语言模型（LLM）驱动的多模态智能体正越来越多地被部署，通过持续的音频交互来执行自主任务。虽然这种范式增强了交互的自然性，但它引入了一个关键且尚未被充分探索的攻击面，因为音频输入不可避免地包含用户无法控制的环境噪声。在本文中，我们研究了针对多模态智能体的并发音频提示注入攻击。与传统的针对语音设备的声学攻击不同，我们提出了用于指令增强和场景隐藏的新技术。这些方法允许恶意音频指令在不被察觉的情况下“寄生”在用户语音中，从而劫持智能体执行恶意操作。为了系统地量化这一威胁，我们构建了 AudioAgentSecurity，这是首个针对音频指令注入攻击的全面基准测试，涵盖了 8 个真实任务场景和 10 种不同的攻击模式。我们评估了 11 个最先进的智能体，包括 Gemini 3 Pro 和 GPT-4o-audio。值得注意的是，我们的方法针对先进的 Gemini 3 Pro 实现了 69.10% 的平均攻击成功率（ASR）。为了应对这一威胁，我们进一步引入了级联音频解耦与验证（CADV），这是一种基于声源分离和一致性分析的防御机制。与现有的提示级防御相比，CADV 实现了高达 96% 的检测准确率，并为广泛的声学注入攻击提供了有效保护。最后，在多种动态真实场景下，通过志愿者在豆包 AI 手机上进行的真实世界实验，证实了该攻击的高隐蔽性和有效性，同时证明了我们的防御机制能够可靠地缓解这些漏洞。
