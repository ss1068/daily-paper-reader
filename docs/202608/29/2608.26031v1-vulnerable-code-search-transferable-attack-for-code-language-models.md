...
## 摘要
可靠的代码检索对于提高开发人员的生产力和有效的代码重用至关重要。然而，目前驱动搜索工具的神经代码语言模型（CLMs）容易受到针对非功能性文本元素的对抗性攻击。在本文中，我们介绍了一种与编程语言无关的可迁移对抗性攻击，利用了 CLM 的这一漏洞。我们的方法在不改变代码片段功能的情况下扰动其中的标识符，从而人为地使代码与目标查询对齐。我们证明，即使是使用较小的代码嵌入模型（如 CodeT5+）计算出的攻击，也具有高度有效性，并且可以迁移到更大的闭源嵌入模型（如 Voyage-code-3）或大语言模型（如 Gemini-3.1-Pro）。我们的攻击可以增加查询与任意无关代码片段之间的相似度，从而使最先进模型的平均倒数排名（MRR）等关键检索指标下降高达 77%。实验结果突显了当前代码搜索方法的脆弱性，并强调了对更鲁棒、具备语义感知能力的方法的需求。
