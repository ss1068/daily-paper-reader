...
title_zh: IH-Benchmark：针对大语言模型应用中指令层级鲁棒性的以冲突为中心的基准测试
authors: "Conor McCauley, Zeliang Kan, Jason Martin"
date: 2026-07-28
pdf: "https://arxiv.org/pdf/2607.25987v2"
tags: ["query:llm"]
score: 8.0
evidence: 指令层级鲁棒性与安全
tldr: "本研究针对大语言模型在处理不同优先级指令冲突时的鲁棒性，提出了IH-Benchmark。该基准涵盖系统-用户（S>U）和用户-工具（U>T）两种冲突场景，包含44类约束族和5个应用领域。通过对37个模型的评估，揭示了模型在不同冲突面上的表现差异，强调了指令层级鲁棒性需进行多维度评估的重要性。"
selection_source: fresh_fetch
motivation: 现有基准无法全面评估大语言模型在面对来自系统、用户和工具的冲突指令时的优先级遵循能力。
method: 构建了一个包含44种约束类型的分类体系，并采用谓词DSL与LLM评判器相结合的二元评估协议。
result: "评估显示模型在S>U和U>T场景下的表现并不一致，且许多模型在处理工具输出中的冲突指令时表现较差。"
conclusion: 指令层级鲁棒性并非单一能力，必须针对不同的冲突表面、约束类型和攻击形式进行全面评估。
---

## 摘要
当语言模型接收到来自不同优先级层级的冲突指令时，它实际上会遵循哪一个？这个问题是可靠大语言模型（LLM）部署的核心。现有的基准测试仅部分回答了这一问题，通常侧重于单一的层级边缘，或者改编自工具使用覆盖范围有限的公开数据集。我们提出了 IH-Benchmark，这是一个以冲突为中心的基准测试，用于评估跨直接系统-用户冲突 (S>U) 和工具介导的用户-工具冲突 (U>T) 的指令层级鲁棒性。IH-Benchmark 基于人工编写的包含 44 个约束族的分类法构建，涵盖通用、医疗、金融、零售和编程场景，并使用统一的二元通过/失败协议评估场景，该协议结合了谓词 DSL 和特定类别的 LLM 裁判。在 37 个评估模型中，层级合规率从 98.2% 到 20.5% 不等。我们发现，强大的 S>U 合规性并不是 U>T 鲁棒性的可靠指标：一些模型在直接用户冲突下能维持系统约束，但当冲突指令出现在工具输出中时，性能会急剧下降。约束强化也揭示了模型之间的分歧：一些失败在很大程度上可以通过更强的警告来修复，而另一些则在所有严格级别下持续存在。最后，最具启发性的失败通常是微妙的，而非明显的危险；模型在抵制未经授权的购买或批量关闭工单方面，比抵制注入的免责声明或细微的事实扭曲更可靠。这些结果表明，指令层级鲁棒性不是一种单一的能力，而是一组必须跨冲突面、约束类型和攻击表现形式进行评估的行为。
