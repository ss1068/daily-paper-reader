...
tldr: 本研究探讨了基于大语言模型的多智能体系统（MAS）中的安全问题。针对现有基于嵌入的防御机制易被恶意智能体通过构造相似嵌入消息绕过的缺陷，本文提出了三种攻击手段并揭示了其局限性。为此，研究者提出利用Token级别的置信度分数（Logits）来识别并过滤恶意信息。实验证明，该方法在多种模型和通信拓扑下均能显著提升系统的鲁棒性，并强调了早期干预在防御中的重要性。
selection_source: fresh_fetch
motivation: 现有的基于嵌入的防御机制无法有效识别那些在向量空间中与良性消息高度相似的恶意攻击。
method: 提出利用大语言模型输出的Token级置信度分数（Logits）来对通信过程中的可疑消息进行剪枝或降权。
result: 实验结果显示，该方法在不同数据集和通信拓扑结构中均能有效提升多智能体系统的抗攻击能力。
conclusion: 相比单纯依赖文本嵌入，Token级别的置信度信号能提供更有效的安全防御，且防御效果随通信轮次增加而衰减，需尽早干预。
---

## 摘要
由大语言模型（LLM）驱动的多智能体系统（MAS）使智能体能够进行通信和共享信息，从而在复杂任务上取得强大的性能。然而，这种通信也产生了一个攻击面，恶意智能体可以传播错误信息并操纵群体决策，从而破坏 MAS 的安全性。现有的基于嵌入的防御旨在检测并剔除可疑智能体，但其有效性取决于恶意消息和良性消息的文本嵌入之间是否存在清晰的分离。攻击者可以通过精心构造嵌入与良性消息接近的消息来规避此类防御。我们从理论上分析了这种失效模式，并通过三种攻击（Slow Drift、Benign Wrapper 和 Chaos Seeding）进行了实证验证。我们的分析进一步揭示了基于嵌入的防御的一个根本局限性：由于它们仅依赖于文本嵌入，因此忽略了诸如 Logits 之类的 Token 级置信度信号，而当嵌入在攻击下无法区分时，这些信号仍能提供信息。我们建议在 MAS 通信过程中使用置信度分数来剔除或降低消息的权重。实验表明，该方法在不同模型、数据集和通信拓扑结构中均提高了鲁棒性。此外，我们发现置信度信号的有效性会随着通信轮次的增加而衰减，这突显了早期干预的重要性。这些见解可以为未来关于 MAS 攻击与防御的研究提供参考和启发。
