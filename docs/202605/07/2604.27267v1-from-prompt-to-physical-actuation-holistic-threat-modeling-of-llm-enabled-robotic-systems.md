...
motivation: 现有研究未能系统性地分析网络、对抗性和对话式威胁如何在LLM驱动的机器人架构中传播并最终导致物理安全后果。
method: 采用分层数据流图对边缘云架构下的机器人系统进行建模，并结合STRIDE框架对六个关键交互点进行多维度威胁分析。
result: 识别出三条从外部输入到危险物理执行的攻击链，揭示了语义验证缺失、跨模态翻译漏洞及不受控工具调用等架构缺陷。
conclusion: 本研究首次实现了全链路威胁整合分析，强调了在LLM机器人系统中建立独立语义验证和受控边界交互的紧迫性。
---

## 摘要
随着大语言模型（LLM）被集成到自主机器人系统中用于任务规划和控制，受损的输入或不安全的模型输出可能会通过规划流水线传播，从而导致物理世界的后果。尽管先前的研究分别探讨了机器人网络安全、对抗性感知攻击和 LLM 安全性，但尚无研究在统一的架构模型中追踪这些威胁类别如何跨越信任边界进行交互和传播。我们通过将边缘云架构中的 LLM 赋能自主机器人建模为分层数据流图（DFD），并利用包含传统网络威胁、对抗性威胁和对话性威胁的三类分类法，对六个跨边界交互点应用了 STRIDE 逐交互分析，从而填补了这一空白。分析表明，这些类别在相同的边界交叉点汇合。我们追踪了从外部入口点到不安全物理执行的三条跨边界攻击链，每条攻击链都揭示了一个独特的架构属性：用户输入与执行器调度之间缺乏独立的语义验证、从视觉感知到语言模型指令的跨模态转换，以及通过提供商端工具调用实现的无中介边界跨越。据我们所知，这是首个在 LLM 赋能机器人系统的完整“感知-规划-执行”流水线中集成所有三类威胁的基于 DFD 的威胁分析。
